    ...
    class TableHelper {
        ...
        public static function getWomenTableDates($query, $page, $filter) {
            ...
            if (($query === null || $query === 'null') && ($filter === null || $filter === 'null')) { //Vamos a hacer la instancia normal de cuando carga la página
                $stmt = $conn->prepare("SELECT id, nombre, apellidos, CAST(AES_DECRYPT(dni,'xyz123') AS CHAR) AS dni, CAST(AES_DECRYPT(fecha_nac,'xyz123') AS CHAR) AS fecha_nac, CAST(AES_DECRYPT(direccion,'xyz123') AS CHAR) AS direccion, CAST(AES_DECRYPT(expediente, 'xyz123') AS CHAR) AS expediente FROM mujeres ORDER BY nombre LIMIT 50 offset $offset");
                ...
                $stmt = $conn->prepare("SELECT id, nombre, apellidos, CAST(AES_DECRYPT(dni,'xyz123') AS CHAR) AS dni, CAST(AES_DECRYPT(fecha_nac,'xyz123') AS CHAR) AS fecha_nac, CAST(AES_DECRYPT(direccion,'xyz123') AS CHAR) AS direccion, CAST(AES_DECRYPT(expediente, 'xyz123') AS CHAR) AS expediente FROM mujeres $query ORDER BY nombre LIMIT 50 offset $offset");
            } else if ($query !== null && $filter !== null) { //Este es el caso de que use un filtro dentro de la paginacion
                $stmt = $conn->prepare("SELECT id, nombre, apellidos, CAST(AES_DECRYPT(dni,'xyz123') AS CHAR) AS dni, CAST(AES_DECRYPT(fecha_nac,'xyz123') AS CHAR) AS fecha_nac, CAST(AES_DECRYPT(direccion,'xyz123') AS CHAR) AS direccion, CAST(AES_DECRYPT(expediente, 'xyz123') AS CHAR) AS expediente FROM mujeres $query ORDER BY $filter LIMIT 50 offset $offset");
            }
            //echo "Consulta SQL: " . $stmt->queryString . "<br>"; //Esto es para ver la consulta que me devuelve
            $stmt->execute();
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $result;
        }
        ...
        public static function countGetWomenTableDates($query) {
            ...
            if ($query === null) 
                ...
                $stmt = $conn->prepare("SELECT COUNT(nombre) AS count FROM mujeres $query");
